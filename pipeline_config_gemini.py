"""Best‑prompts pipeline configuration for Gemini 2.5 Pro (runner‑compatible)
...
• Uses Google Search on every LLM step; snapshot handled locally by the runner.
"""
...
- If no credible source is found with Google Search, OMIT the claim entirely.
...
PIPELINE_STEPS = [
    ...
    StepConfig(
        ...
        prompt_template=(
            ...
            "TASK\n"
            "- Use Google Search for experimentally supported, DIRECT interactors of the MAIN protein.\n"
    ...
    StepConfig(
        ...
        prompt_template=(
            ...
            "FOR EACH interactor entry, USE GOOGLE SEARCH to find functions DIRECTLY CAUSED by that interaction (respect direction).\n"
    ...
    StepConfig(
        ...
        deep_research=False,
        reasoning_effort="high",
        use_google_search=True,  # keep ON for re-verification
        thinking_budget=MAX_THINKING_TOKENS,
        max_output_tokens=MAX_OUTPUT_TOKENS,
        search_dynamic_mode=True,
        search_dynamic_threshold=DYNAMIC_SEARCH_THRESHOLD,
